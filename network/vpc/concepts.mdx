...
A resource's complete IP address on a given Private Network will therefore look something like `172.16.16.2/22`. This encapsulates information both about the resource's IP address, and also which subnet (Private Network) it is from. If you configure an Instance manually with this address, it sets both the correct IP and a route to the correct subnet.
